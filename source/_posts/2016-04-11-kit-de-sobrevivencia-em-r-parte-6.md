...
No último post, você aprendeu um pouco sobre os tipos de dados e como realizar algumas transformações. Neste post, trataremos um pouco sobre estruturas de controles. Estruturas de controles serão bastantes usadas durante o procesos de análise de dados, sendo importante que você domine esse tópico.
...
Antes de criar o plot, vamos realizar um exemplo simples para exemplificar com `for()` funciona.
...
Agora, vamos definir alguns parâmetros do plot. Aqui usaremos o recurso base do R para geração dos gráficos. Atualmente, uma boa parte dos usuários (inclusive a gente!) utiliza o [ggplot2](http://docs.ggplot2.org/current/) e com um pouco de paciência e uma boa base de dados você poderá criar gráficos como o que está [neste post](https://medium.com/airbnb-engineering/using-r-packages-and-education-to-scale-data-science-at-airbnb-906faa58e12d#.z39ukskpb) do Airbnb. Os parâmetros do plot serão definidos usando a função `par()`. Utilize o `?par` para ver mais detalhes sobre esta função e as opções disponíveis.
...
* Ordenar as linhas dos `dados.tmp` de forma decrescente pelo valor da taxa de desemprego;
...
O `while()` é uma estrutura de controle utilizada para realizar um loop enquanto uma condição for satisfeita. Aqui realizaremos um loop simple somente para demonstrar o funcionamento do `while()`. Será criado um objeto `i` igual 1 e iremos mostrá-lo no console enquanto for menor ou igual a 5. A cada iteração será adicionado 1 ao valor de `i`. 
...
Como você notou no exemplo do `if` e `else`, o R permite que um loop seja elaborado com várias estruturas de controle. Usaremos isto para exemplificar o funcionamento do `repeat` e do `break`. Além disos, também utilizaremos o `if`. O `repeat` realiza as operações que estão entre `{}` indefinidamente. Obviamente, quase sempre não iremos querer que o loop não termine nunca. Dessa forma, temos que criar uma condição para que ele pare e um instrução dizendo que o loop deve terminar caso a condição seja satisfeita. 
...
repeat{
  ...
  i <- i + 1
  if(i > 6){
    break # Se a condição a satisfeita, para o loop.
  }
}
{% endhighlight %}
