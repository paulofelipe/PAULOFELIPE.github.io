...
O `while()` é uma estrutura de controle utilizada para realizar um loop enquanto uma condição for satisfeita. Aqui realizaremos um loop simples somente para demonstrar o funcionamento do `while()`. Será criado um objeto `i` igual 1 e iremos mostrá-lo no console enquanto for menor ou igual a 5. A cada iteração será adicionado 1 ao valor de `i`. 
...
Perceba que não há necessidades das `{}` se a operação a ser executada em um estrutura de controle estiver na mesma linha. Você também poderia usar normalmente o `{}`.
...
Use como critério de parada se $f(x_n) < 0.000001$ e como valor inicial $x_0 = 3$.
